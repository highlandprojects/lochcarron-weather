...
function updateSevenDayOutlook() {
  ...
  $("sevenTides").innerHTML = tideDays.map((date) => {
    const tides = tideWindows(date);
    const tideLabel = tides.map((t) => `${t.type} ${timeLabel(t.time)} ${t.height.toFixed(1)}m est.`).join(", ");
    return `
      <div class="forecastcard tideforecast" title="${dayName(date)}: ${tideLabel}">
